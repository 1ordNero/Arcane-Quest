...
function icon(src){return `<span class="ds-icon">${img(src,'')}</span>`}
function resources(){
 const parts=[`<span class="ds-chip ds-level">Lv ${Number(S.lvl)||1}</span>`,`<span class="ds-chip ds-gold">${img('assets/icons/ui/resource_gold.webp','Gold')}${Number(S.gold)||0}</span>`];
 if(S.screen==='home')parts.push(`<span class="ds-chip">${img('assets/icons/ui/resource_energy.webp','Abenteuerlust')}${Number(S.al)||0}/${Number(S.maxAl)||100}</span>`);
 else if(S.screen==='shrine')parts.push(`<span class="ds-chip">${img('assets/icons/ui/ui_soulstone.webp','Seelensteine')}${Number(S.souls)||0}</span>`);
 else if(['city','merchant','bank','forge'].includes(S.screen))parts.push(`<span class="ds-chip ds-bag">${img('assets/icons/ui/ui_inventory.webp','Inventar')}${(S.items||[]).length}/${Number(S.invCap)||15}</span>`);
 else if(S.screen==='arena')parts.push(`<span class="ds-chip">Ruhm ${Number(S.arena)||0}</span>`);
 return parts.join('');
}
function xpBar(){
 const p=window.ARCANE_PROGRESSION?.progress?.(S);
 if(p){if(p.capped)return `<div class="ds-xp"><span>STUFE ${p.maxLevel}</span><i><u style="width:100%"></u></i><b>MAX</b></div>`;return `<div class="ds-xp"><span>XP</span><i><u style="width:${Math.max(0,Math.min(100,p.pct||0))}%"></u></i><b>${p.xp} / ${p.need}</b></div>`}
 if(typeof window.xpNeed!=='function')return '';
 const raw=Number(window.xpNeed())||0;if(raw<=0)return '<div class="ds-xp"><span>XP</span><i><u style="width:100%"></u></i><b>MAX</b></div>';
 const xp=Math.max(0,Number(S.xp)||0),pct=Math.max(0,Math.min(100,xp/raw*100));return `<div class="ds-xp"><span>XP</span><i><u style="width:${pct}%"></u></i><b>${xp} / ${raw}</b></div>`;
}
function header(){const h=document.querySelector('header');if(!h)return;const m=SCREEN_META[S.screen]||{icon:'assets/icons/app-icon-192.webp',title:'Arcane Quest',sub:''};h.className='ds-header';h.innerHTML=`<div class="ds-top"><div class="ds-screen">${icon(m.icon)}<div><b>${esc(m.title)}</b><small>${esc(m.sub)}</small></div></div><div class="ds-res">${resources()}</div></div>${S.screen==='char'?'':xpBar()}`}
function removeRedundancy(){document.body.dataset.screen=S.screen||'';if(S.screen==='city')document.querySelectorAll('.cv2-head,.compact-city-status').forEach(x=>x.classList.add('ds-hide-duplicate'));if(['merchant','bank','forge','shrine'].includes(S.screen)){document.querySelectorAll('.cux-top').forEach(x=>x.classList.add('ds-subnav'));const head=document.querySelector(S.screen==='forge'?'.fv4-head':S.screen==='shrine'?'.sv3-head':'.mb2-head');if(head)head.classList.add('ds-secondary-head')}}
function polish(){header();removeRedundancy();document.querySelectorAll('.card').forEach(x=>x.classList.add('ds-card'))}
window.Arcane?.on?.('afterRenderSettled',polish);window.Arcane?.on?.('bootReady',polish);
...
`;document.head.appendChild(css);queueMicrotask(polish);
})();