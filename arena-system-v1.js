...
function step(){const f=S.arenaV2?.fight;if(!f||f.done)return;const st=stats(),stance=f.stance;f.round++;let dmg=Math.max(2,Math.round((st.damage||4)+(st.str+st.agi+st.int)/5+Math.random()*8));const crit=(st.crit||0)+(stance==='aggressive'?15:0);if(stance==='defensive')dmg=Math.round(dmg*.9);if(Math.random()*100<crit){dmg=Math.round(dmg*1.65);f.log.push(`Kritischer Treffer: ${dmg} Schaden.`)}else f.log.push(`Du verursachst ${dmg} Schaden.`);f.ehp=Math.max(0,f.ehp-dmg);if(f.ehp<=0)return finish(true);let enemyDamage=Math.max(2,Math.round(5+f.o.power/35+Math.random()*7));const dodge=(st.dodge||0)+(stance==='counter'?15:0);if(Math.random()*100<dodge){f.log.push(`Du weichst ${f.o.name}s Angriff aus.`);if(stance==='counter'){const counter=Math.max(2,Math.round(dmg*.45));f.ehp=Math.max(0,f.ehp-counter);f.log.push(`Konter: ${counter} Schaden.`);if(f.ehp<=0)return finish(true)}}else{const armor=(st.armor||0)*(stance==='defensive'?1.2:stance==='aggressive'?.9:1);enemyDamage=Math.max(1,Math.round(enemyDamage-armor*.12));f.php=Math.max(0,f.php-enemyDamage);f.log.push(`${f.o.name} trifft dich für ${enemyDamage}.`)}if(f.php<=0)return finish(false);f.log=f.log.slice(-8);save();render();setTimeout(step,650)}
