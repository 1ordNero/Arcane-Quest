...
function place(){
 ...
 layer.classList.remove('aq-ob-ready');
 ...
 if(!focus||focus.hidden){card.style.left=`${Math.max(MARGIN,(vw-cw)/2)}px`;card.style.top=`${MARGIN}px`;requestAnimationFrame(()=>layer.classList.add('aq-ob-ready'));return}
 ...
 if(freeBelow>=Math.min(desiredHeight,250)){card.style.top=`${fr.bottom+GAP}px`;card.style.maxHeight=`${Math.max(220,freeBelow)}px`;requestAnimationFrame(()=>layer.classList.add('aq-ob-ready'));return}
 if(freeAbove>=Math.min(desiredHeight,250)){const h=Math.min(desiredHeight,freeAbove);card.style.top=`${Math.max(MARGIN,fr.top-GAP-h)}px`;card.style.maxHeight=`${Math.max(220,h)}px`;requestAnimationFrame(()=>layer.classList.add('aq-ob-ready'));return}
 ...
 requestAnimationFrame(()=>layer.classList.add('aq-ob-ready'));
}
let queued=false;function schedule(){if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;place()})}
...
html:has(body.aq-onboarding-active),body.aq-onboarding-active{scroll-behavior:auto!important}
.aq-ob-layer .aq-ob-card,.aq-ob-layer .aq-ob-focus{opacity:0!important;visibility:hidden!important;transition:none!important}
.aq-ob-layer.aq-ob-ready .aq-ob-card,.aq-ob-layer.aq-ob-ready .aq-ob-focus{opacity:1!important;visibility:visible!important}
.aq-ob-layer.aq-ob-ready .aq-ob-card{transition:opacity .08s linear!important}
.aq-ob-layer.aq-ob-ready .aq-ob-focus{transition:opacity .08s linear!important}
.aq-ob-card{display:flex!important;flex-direction:column!important;overflow:hidden!important;overscroll-behavior:contain!important;overflow-anchor:none!important;max-height:calc(100dvh - 20px)!important;will-change:opacity}
.aq-ob-card .aq-ob-progress,.aq-ob-card .aq-ob-eyebrow,.aq-ob-card h2{flex:0 0 auto}
.aq-ob-copy-scroll{min-height:0!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch!important;overscroll-behavior:contain!important;overflow-anchor:none!important;padding-right:4px!important;scrollbar-width:thin;scrollbar-color:#a875ff66 transparent}
...
.aq-ob-focus{pointer-events:none!important;will-change:opacity}
...
@media(prefers-reduced-motion:reduce){.aq-ob-layer.aq-ob-ready .aq-ob-card,.aq-ob-layer.aq-ob-ready .aq-ob-focus{transition:none!important}}
