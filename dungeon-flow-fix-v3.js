...
window.dungeonV1=function(){let h=oldView();const d=S.dungeonV1;if(!d)return h;
 ...
 if(sp){const sectionEnd=h.lastIndexOf('</section>');if(sectionEnd>=0)h=h.slice(0,sectionEnd)+sp+h.slice(sectionEnd)}
 return h;
};
// Several older modules captured their own dungeon view inside render().
// This final wrapper deliberately renders the current public dungeonV1 view last.
const previousRender=window.render;
window.render=function(){previousRender();if(S.screen==='dungeon'){const main=document.querySelector('main');if(main)main.innerHTML=window.dungeonV1()}};
