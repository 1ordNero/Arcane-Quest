...
function state(){const s=window.S;if(!s)return null;if(!s[STORAGE_KEY]||typeof s[STORAGE_KEY]!=='object')s[STORAGE_KEY]={version:VERSION,completed:{},progress:{},dismissed:{},startedAt:Date.now()};const o=s[STORAGE_KEY];o.version=VERSION;o.completed=o.completed||{};o.progress=o.progress||{};o.dismissed=o.dismissed||{};return o}
function save(){Arcane.state?.scheduleSave?.()||window.save?.()}
...
function currentChapter(){const screen=S?.screen||'home';if(CHAPTERS[screen]&&eligible(screen)&&!isDone(screen))return screen;if(screen==='city')return null;return null}
...
let scheduled=false;function evaluate(){if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;if(document.querySelector('.reward-overlay,.aq-dialog,.mb2-modal,.reward-card-overlay'))return;const id=currentChapter();if(id)renderChapter(id);else clear()})}
