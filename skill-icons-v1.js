...
function apply(root=document){
 ...
 root.querySelectorAll?.('.sk-rotation>button').forEach(btn=>{
  const name=btn.querySelector('b')?.textContent?.trim();const src=byName.get(name);const holder=btn.querySelector('span');
  if(src&&holder&&!holder.querySelector('img'))holder.replaceChildren(iconImg(src,name));
 });
 root.querySelectorAll?.('.sk-list>button').forEach(btn=>{
  const name=btn.querySelector('b')?.textContent?.trim();const src=byName.get(name);const holder=btn.querySelector('span');
  if(src&&holder&&!holder.querySelector('img'))holder.replaceChildren(iconImg(src,name));
 });
}
...
window.Arcane=window.Arcane||{};Arcane.skillIcons={assets:ASSETS,apply};
})();
