...
function buildCompactDungeon(){
  ...
  if(!section||section.dataset.compactV2==='1')return;
  section.dataset.compactV2='1';
  const d=S.dungeonV1;

  /* Remove duplicated/secondary status UI created by older dungeon layers. */
  section.querySelectorAll('.dux-run,.d1-bank,.d2-loot,.dux-exit').forEach(x=>x.remove());
