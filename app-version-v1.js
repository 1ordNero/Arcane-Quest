...
const start=()=>{const app=document.getElementById('app');if(app)new MutationObserver(m=>{if(m.some(x=>x.type==='childList'&&[...x.addedNodes].some(n=>n.nodeType===1&&(n.matches?.('header')||n.querySelector?.('header')))))schedule()}).observe(app,{childList:true,subtree:false});apply()};
