const CACHE='arcane-quest-shell-v5';
...
async function staleWhileRevalidate(req){const cached=await caches.match(req);const network=fetch(req).then(res=>put(req,res)).catch(()=>null);return cached||(await network)||Response.error()}
...
self.addEventListener('fetch',event=>{
 ...
 if(isImage(url)){event.respondWith(staleWhileRevalidate(req));return}
