...
const isCanonicalVersionRuntime=url=>url.pathname.endsWith('/release-runtime-v1.js')||url.pathname.endsWith('release-runtime-v1.js');
...
async function codeCacheFirst(req){const c=await caches.open(SHELL_CACHE),hit=await c.match(req);if(hit)return hit;try{return await put(SHELL_CACHE,req,await fetch(req,{cache:'no-cache'}))}catch{return Response.error()}}
...
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET')return;const url=new URL(req.url);if(url.origin!==self.location.origin)return;if(req.mode==='navigate'){event.respondWith(networkFirst(req,'./index.html'));return}if(isCanonicalVersionRuntime(url)){event.respondWith(networkFirst(req));return}if(isImage(url)){event.respondWith(imageCacheFirst(req));return}if(isCode(url)){event.respondWith(codeCacheFirst(req));return}event.respondWith(networkFirst(req))});