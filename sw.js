...
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET')return;const url=new URL(req.url);if(url.origin!==self.location.origin)return;if(req.mode==='navigate'){event.respondWith(networkFirst(req,'./index.html'));return}if(isImage(url)){event.respondWith(imageCacheFirst(req));return}if(isCode(url)){event.respondWith(codeCacheFirst(req));return}event.respondWith(networkFirst(req))});