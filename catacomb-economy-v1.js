...
function introKey(){retireBeta();clampKeys();if((Number(S.lvl)||1)<10||S.catacombKeyIntroGranted)return false;S.catacombKeyIntroGranted=true;if(S.keys<1)S.keys=1;return true}
...
function processQuestResult(){const r=S.questResult;if(!r||r._catacombEconomyV1||processing)return false;const kind=QUEST_KIND[r.name];if(!kind)return false;processing=true;r._catacombEconomyV1=true;let changed=false;if(r.item&&ITEM_KEEP[kind]!=null&&Math.random()>ITEM_KEEP[kind]){removeQuestItem(r.item);r.item=null;r.itemRarity=null;r.itemStats=null;changed=true}if((Number(S.lvl)||1)>=10&&S.keys<KEY_CAP){S.catacombKeyPity=Math.max(0,Number(S.catacombKeyPity)||0)+1;const guaranteed=S.catacombKeyPity>=4;if(guaranteed||Math.random()<(KEY_CHANCE[kind]||0)){S.keys=Math.min(KEY_CAP,S.keys+1);S.catacombKeyPity=0;r.keys=1;changed=true;log?.(`Katakomben-Schlüssel erhalten (${S.keys}/${KEY_CAP}).`)}}if(changed||kind)save?.();processing=false;return changed}
...
function decorateHeader(){const old=document.querySelector('.ce1-key-chip');if(S.screen!=='dungeon'){old?.remove();return}const top=document.querySelector('header .top')||document.querySelector('header');if(!top)return;if(old){old.querySelector('b').textContent=String(S.keys||0);return}const el=document.createElement('span');el.className='ce1-key-chip';el.innerHTML=`<small>Schlüssel</small><b>${S.keys||0}</b><em>/${KEY_CAP}</em>`;top.appendChild(el)}
function tick(){ensureState();patchStart();wrapDungeonLoot();const changed=migrateActiveRun();const q=processQuestResult();decorateResult();decorateHeader();if(changed)save?.();if(q)requestAnimationFrame(()=>{decorateResult();decorateHeader()})}
