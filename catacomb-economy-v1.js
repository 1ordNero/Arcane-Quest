...
function removeQuestItem(id,name){if(!id&&!name)return false;const matches=it=>!!it&&(id?it.id===id:it.name===name),items=S.items||[],ix=items.findIndex(matches);if(ix>=0){items.splice(ix,1);return true}for(const [slot,it] of Object.entries(S.eq||{})){if(matches(it)){delete S.eq[slot];window.syncEquipmentStats?.();return true}}return false}
function processQuestResult(){const r=S.questResult;if(!r||r._catacombEconomyV1||processing)return false;const kind=QUEST_KIND[r.name];if(!kind)return false;processing=true;r._catacombEconomyV1=true;let changed=false;if(r.item&&ITEM_KEEP[kind]!=null&&Math.random()>ITEM_KEEP[kind]){removeQuestItem(r.itemId,r.item);r.item=null;r.itemId=null;r.itemSlot=null;r.itemRarity=null;r.itemStats=null;changed=true}if((Number(S.lvl)||1)>=10){S.catacombKeyPity=Math.max(0,Number(S.catacombKeyPity)||0)+1;const guaranteed=S.catacombKeyPity>=4;if(guaranteed||Math.random()<(KEY_CHANCE[kind]||0)){S.keys=Math.max(0,Number(S.keys)||0)+1;S.catacombKeyPity=0;r.keys=1;changed=true;log?.(`Katakomben-Schlüssel erhalten (${S.keys}).`)}}if(changed||kind)save?.();processing=false;return changed}
