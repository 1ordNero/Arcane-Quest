...
function decorateArena(){
  ...
  const root=document.querySelector('.av2');if(!root||root.querySelector('.arena-stamina'))return;
  const remaining=S.arenaStamina;
  const box=document.createElement('div');box.className=`arena-stamina ${remaining<=0?'empty':''}`;
...
if(oldRender)window.render=function(){ensureArenaStamina();const r=oldRender.apply(this,arguments);decorateXP();decorateArena();return r};
...
`;document.head.appendChild(css);ensureArenaStamina();decorateXP();decorateArena();
})();
