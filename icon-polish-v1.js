...
function polish(){
  ...
  if(['city','merchant','bank','forge'].includes(S.screen)){
    const cityTitle=document.querySelector('.cux-city-title>span');
    if(cityTitle)cityTitle.innerHTML=img('assets/icons/nav-stadt.webp');
    const crumb=document.querySelector('.cux-top button');
    if(crumb&&crumb.textContent.includes('Stadt'))crumb.innerHTML=`${img('assets/icons/nav-stadt.webp','ui-inline-icon')}<span>Stadt</span>`;
...
window.render=function(){const r=prev.apply(this,arguments);polish();return r};
