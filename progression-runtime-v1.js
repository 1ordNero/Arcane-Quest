...
function levelOf(state=window.S){return Math.max(1,Math.min(MAX_LEVEL,Math.floor(Number(state?.lvl)||1)))}
...
function xpNeedFor(state=window.S){const lvl=levelOf(state);return lvl>=MAX_LEVEL?0:xpForLevel(lvl)}
function normalize(state=window.S){if(!state)return state;const raw=Math.max(1,Math.floor(Number(state.lvl)||1));if(raw>MAX_LEVEL)state.lvl=MAX_LEVEL;if(Number(state.lvl)>=MAX_LEVEL){state.lvl=MAX_LEVEL;state.xp=0}else state.xp=Math.max(0,Math.floor(Number(state.xp)||0));return state}
function gain(amount,state=window.S){if(!state)return 0;normalize(state);if(state.lvl>=MAX_LEVEL){state.xp=0;return 0}const n=Math.max(0,Math.floor(Number(amount)||0));state.xp+=n;let levels=0;while(state.lvl<MAX_LEVEL){const need=xpForLevel(state.lvl);if(state.xp<need)break;state.xp-=need;state.lvl++;levels++;state.maxHp=(Number(state.maxHp)||120)+10;state.hp=state.maxHp;state.str=(Number(state.str)||0)+1;state.agi=(Number(state.agi)||0)+1;state.int=(Number(state.int)||0)+1}if(state.lvl>=MAX_LEVEL){state.lvl=MAX_LEVEL;state.xp=0}return levels}
function progress(state=window.S){normalize(state);const lvl=levelOf(state);if(lvl>=MAX_LEVEL)return{level:lvl,maxLevel:MAX_LEVEL,xp:0,need:0,pct:100,capped:true};const need=xpForLevel(lvl),xp=Math.max(0,Number(state?.xp)||0);return{level:lvl,maxLevel:MAX_LEVEL,xp,need,pct:Math.min(100,xp/Math.max(1,need)*100),capped:false}}
...
window.xpNeed=()=>xpNeedFor(window.S);
window.gainXP=n=>gain(n,window.S);
root.on?.('beforeSave',()=>normalize(window.S));
normalize(window.S);
})();