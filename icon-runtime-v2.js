...
const ASSETS={
 ...
 merchant:UI+'location_merchant.webp',forge:UI+'location_forge.webp',bank:UI+'location_bank.webp',shrine:UI+'location_shrine.webp',trainer:GAME+'city_trainer.webp',
...
function ensure(host,src,cls,alt=''){
 ...
 const image=document.createElement('img');image.src=src;image.className=cls;image.alt=alt;image.decoding='async';image.loading='lazy';image.dataset.arcaneAssetSource=src;
...
function citySrc(text=''){
 if(/Ahnenschrein|Schrein/i.test(text))return ASSETS.shrine;
 if(/Ahnenschmiede|Schmiede/i.test(text))return ASSETS.forge;
 if(/Händler/i.test(text))return ASSETS.merchant;
 if(/Bank/i.test(text))return ASSETS.bank;
 return ASSETS.trainer;
}
function city(){
 document.querySelectorAll('.cv2-grid>button').forEach(btn=>{const label=btn.querySelector('b')?.textContent?.trim()||btn.textContent||'';ensure(btn.querySelector(':scope > .cv2-icon')||btn.querySelector(':scope > span:first-child'),citySrc(label),'aq-icon aq-icon-city',label)});
}
...
.cv2-grid>button>.cv2-icon,.cv2-grid>button>span:first-child{position:relative!important;display:grid!important;place-items:center!important;width:48px!important;height:48px!important;min-width:48px!important;min-height:48px!important;overflow:hidden!important;align-self:center!important}
.cv2-grid>button>.cv2-icon>.aq-icon-city,.cv2-grid>button>span:first-child>.aq-icon-city{width:42px!important;height:42px!important}
...
@media(max-width:520px){.quest-card .q-icon,.active-quest .q-icon{width:58px!important;height:58px!important;min-width:58px!important;min-height:58px!important;flex-basis:58px!important}.quest-card .q-icon>.aq-icon-quest,.active-quest .q-icon>.aq-icon-quest{width:48px!important;height:48px!important}.cv2-grid>button>.cv2-icon,.cv2-grid>button>span:first-child{width:44px!important;height:44px!important;min-width:44px!important;min-height:44px!important}.cv2-grid>button>.cv2-icon>.aq-icon-city,.cv2-grid>button>span:first-child>.aq-icon-city{width:38px!important;height:38px!important}.av2-stances button b>.aq-icon-stance{width:34px!important;height:34px!important;flex-basis:34px!important}}
