...
function byClass(slot){const c=S.cls||'Krieger';slot=norm(slot);
 ...
 if(slot==='Zweithand')return c==='Krieger'?'shield-knight.webp':c==='Magier'?NEW.mageOffhand:c==='Hexenmeister'?NEW.warlockOffhand:c==='Druide'?NEW.druidOffhand:'shield-knight.webp';
 ...
 if(slot==='Schulter')return c==='Krieger'?NEW.warriorShoulders:c==='Druide'?'shoulders-druid-01.webp':c==='Magier'?'shoulders-mage-01.webp':'shoulders-warlock-01.webp';
...
let raf=0;
function heroDecorate(){if(typeof S==='undefined'||S.screen!=='char')return;document.querySelectorAll('.he4-slot.has').forEach(btn=>{const slot=norm(btn.querySelector('small')?.textContent.trim()),it=S.eq?.[slot]||S.eq?.[slot==='Schulter'?'Schultern':slot==='Zweithand'?'Nebenhand':slot==='Ring'?'Ring 1':slot],host=btn.querySelector(':scope > span');if(it&&host&&!host.querySelector('img'))host.innerHTML=tag(it,slot,'ia2-hero')});document.querySelectorAll('.he4-item').forEach(btn=>{const name=btn.querySelector('b')?.textContent.replace(/\s+\+\d+$/,'').trim(),it=(S.items||[]).find(x=>x.name===name),host=btn.querySelector('.he4-itemicon');if(it&&host&&!host.querySelector('img'))host.innerHTML=tag(it,null,'ia2-list')});document.querySelectorAll('.he4-detail').forEach(box=>{const name=box.querySelector('h2')?.textContent.replace(/\s+\+\d+$/,'').trim(),it=[...(S.items||[]),...Object.values(S.eq||{}).filter(Boolean)].find(x=>x.name===name),host=box.querySelector('.he4-icon');if(it&&host&&!host.querySelector('img'))host.innerHTML=tag(it,null,'ia2-detail')})}
function reconcile(){if(raf)return;raf=requestAnimationFrame(()=>{raf=0;heroDecorate()})}
window.Arcane?.on?.('afterRenderSettled',reconcile);window.addEventListener('arcane:hero-layout-ready',reconcile);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',reconcile,{once:true});else reconcile();
const css=document.createElement('style');css.textContent=`.ia2-hero,.ia2-list,.ia2-detail,.fv3-art,.mb2-art{display:block;width:100%;height:100%;object-fit:contain;pointer-events:none}.he4-slot.has>span{width:52px!important;height:52px!important}.he4-itemicon,.he4-icon,.he4-slot.has{overflow:hidden}.ia2-list,.ia2-detail{transform:scale(1.28)}.ia2-hero{transform:scale(1.34);transform-origin:center}.fv3-art,.mb2-art{transform:scale(1.22);transform-origin:center}`;document.head.appendChild(css);
})();